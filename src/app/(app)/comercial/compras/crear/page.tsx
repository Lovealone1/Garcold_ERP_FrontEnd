...
import {
    useSupplierOptions,
    type SupplierOption,
} from "@/hooks/proveedores/useSupplierOptions";
...
export default function CompraCrearPage() {
    ...
    const { options: estadoOptions, byName: estadoByName } =
        useCompraEstados();
    ...
    const [proveedorSel, setProveedorSel] =
    ...
    const [purchaseAt, setPurchaseAt] = useState<string | undefined>(
        undefined,
    );

    useEffect(() => {
        if (!bancoSel && bancos.length) {
            const eff = bancos.find((b) =>
                b.name?.toLowerCase().includes("efectivo"),
            );
    ...
    useEffect(() => {
        if (!estadoSel && estadoOptions.length) {
            const target = estadoOptions.find(
                (n) =>
                    COMPRA_CONTADO_RE.test(String(n)) &&
                    !BLOQUEADA_RE.test(String(n)),
            );
    ...
    function setQty(idTmp: string, v: number) {
        const q = clamp(
            Number.isFinite(v) ? Math.trunc(v) : 1,
            1,
            1e9,
        );
        setItems((prev) =>
            prev.map((it) =>
                it.idTmp === idTmp ? { ...it, cantidad: q } : it,
            ),
        );
    }

    const onConfirmProducto = (data: {
        precio_unitario: number;
        cantidad: number;
    }) => {
              ...
              `,
                ...
                >
                    ...
                    <Box sx={{ gridArea: "estado", minWidth: { xs: 0, md: 180 } }}>
                        ...
                        <Autocomplete
                            ...
                            renderInput={(params) => (
                                <TextField
                                    {...params}
                                    placeholder="Compra contado / crédito…"
                                    sx={textFieldSx}
                                />
                            )}
                        />
                    ...
                    >
                        ...
                        {/* Autocomplete + botón de escáner (mismo layout que ventas) */}
                        <Box
                            sx={{
                                mt: 0.5,
                                display: "flex",
