...
function BienvenidoInner() {
    ...
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            ...
            tl.set(logoRef.current, { opacity: 0, scale: 0.6 })
                .to(logoRef.current, { opacity: 1, scale: 1.2, duration: 1.1 })
                .to(logoRef.current, { scale: 1.32, duration: 0.55, ease: "power2.inOut" }, "+=0.25")
        ...
        >
            ...
            <div ref={textWrapRef} style={{ display: phase === "text" ? "block" : "none" }} className="px-4">
                <SplitText
                    ...
                    tag="h1"
                    className="font-extrabold leading-[0.98] text-center text-[clamp(64px,12vw,180px)]"
                    splitType="chars"
                    delay={0.09 * 1000}
                    duration={0.9}
                    ease="expo.out"
