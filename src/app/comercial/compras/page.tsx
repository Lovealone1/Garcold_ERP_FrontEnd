...
export default function ComprasPage() {
    ...
    useEffect(() => {
        setLoading(true);
        fetch(`/api/compras?page=${page}&size=${size}&q=${encodeURIComponent(q)}`)
            .then((r) => r.json())
            .then((res: { items: Compra[]; total: number }) => { setRows(res.items); setTotal(res.total); })
    ...
    const onView = (c: Compra) => { };
    const onDownloadInvoice = (c: Compra) => { };
    const onPay = (c: Compra) => { };
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="bg-[var(--table-head-bg)] text-[var(--table-head-fg)]">
                                    <th className="px-4 py-3 text-left">Nro Compra</th>
                                    ...
                                    ? Array.from({ length: 10 }).map((_, i) => (
                                        <tr key={`sk-${i}`} className="border-t border-tg">
                                            {Array.from({ length: 8 }).map((__, j) => (
                                                <td key={j} className="px-4 py-3"><div className="h-4 w-full animate-pulse rounded bg-black/10 dark:bg-white/10" /></td>
                                            ))}
                                        </tr>
                                    ))
                                    : rows.length === 0 ? (
                                        <tr><td colSpan={8} className="px-4 py-10 text-center text-tg-muted">Sin registros</td></tr>
                                    ) : rows.map((r) => (
                                        <tr key={r.id} className="border-t border-tg hover:bg-black/5 dark:hover:bg-white/5">
                                            ...
                                            <td className="px-2 py-2">
                                                <div className="flex items-center justify-center gap-1">
                                                    ...
                                                    <Tooltip title="Descargar factura" arrow>
                                                        <IconButton size="small" onClick={() => onDownloadInvoice(r)} aria-label="descargar factura">
                                                            <DownloadOutlinedIcon fontSize="small" />
                                                        </IconButton>
                                                    </Tooltip>
                                                    <Tooltip title="Abonar pago" arrow>
                                                        <IconButton size="small" onClick={() => onPay(r)} aria-label="abonar pago">
                                                            <AttachMoneyOutlinedIcon fontSize="small" />
                                                        </IconButton>
                                                    </Tooltip>
                                                    <Tooltip title="Eliminar" arrow>
                                                        <IconButton size="small" onClick={() => onDelete(r)} aria-label="eliminar">
