...
export default function VentasPage() {
    ...
    useEffect(() => {
        setLoading(true);
        fetch(`/api/ventas?page=${page}&size=${size}&q=${encodeURIComponent(q)}`)
            .then((r) => r.json())
            .then((res: { items: Venta[]; total: number }) => { setRows(res.items); setTotal(res.total); })
    ...
    // Acciones
    const onView = (v: Venta) => { };
    const onDownloadInvoice = (v: Venta) => { };
    const onPay = (v: Venta) => { };
