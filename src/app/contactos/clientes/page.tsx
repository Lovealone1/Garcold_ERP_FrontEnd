// app/(comercial)/clientes/page.tsx
"use client";
...
type Cliente = {
    id: number; nombre: string; cc_nit: string;
    correo?: string | null; celular?: string | null;
    direccion: string; ciudad: string;
    saldo: number; fecha_creacion: string;
};
...
export default function ClientesPage() {
    ...
    useEffect(() => {
        setLoading(true);
        fetch(`/api/clientes?page=${page}&size=${size}&q=${encodeURIComponent(q)}`)
            .then((r) => r.json())
            .then((res: { items: Cliente[]; total: number }) => { setRows(res.items); setTotal(res.total); })
    ...
    return (
        <div className="app-shell__frame min-h-screen" style={frameVars}>
            ...
            <div className="bg-[var(--page-bg)] rounded-xl px-[var(--content-x)] pb-[var(--content-b)] pt-3">
                ...
                {/* Contenedor de tabla: más oscuro que el fondo de página */}
                            ...
                            <tbody>
                                {loading
                                    ...
                                    : rows.length === 0
                                        ? (
                                            <tr>
                                                <td colSpan={9} className="px-4 py-10 text-center text-tg-muted">
                                                    Sin registros
                                                </td>
                                            </tr>
                                        )
