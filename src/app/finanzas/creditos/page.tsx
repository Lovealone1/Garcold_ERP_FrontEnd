// app/(finanzas)/creditos/page.tsx
"use client";
...
export default function CreditosPage() {
    ...
    useEffect(() => {
        setLoading(true);
        fetch(`/api/creditos?page=${page}&size=${size}&q=${encodeURIComponent(q)}`)
            .then((r) => r.json())
            .then((res: { items: Credito[]; total: number }) => { setRows(res.items); setTotal(res.total); })
        ...
        <div className="app-shell__frame min-h-screen" style={frameVars}>
            <div className="bg-[var(--page-bg)] rounded-xl px-[var(--content-x)] pb-[var(--content-b)] pt-3">
                <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                    <h2 className="text-2xl font-semibold text-tg-fg">Créditos</h2>
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        <button onClick={onCreate} className="h-10 rounded-md bg-tg-primary px-4 text-sm font-medium text-tg-on-primary shadow-sm">
                            Nuevo crédito
                        </button>
                    </div>
                </div>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="bg-[var(--table-head-bg)] text-[var(--table-head-fg)]">
                                    <th className="px-4 py-3 text-left">Nro Crédito</th>
                            ...
                            <tbody>
                                {loading
                                    ...
                                    : rows.length === 0 ? (
                                        <tr><td colSpan={5} className="px-4 py-10 text-center text-tg-muted">Sin registros</td></tr>
                                    ) : rows.map((r) => (
