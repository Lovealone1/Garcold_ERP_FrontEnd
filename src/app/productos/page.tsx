// app/(comercial)/productos/page.tsx
"use client";
...
export default function ProductosPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/productos?page=${page}&size=${size}&q=${encodeURIComponent(q)}`)
      .then((r) => r.json())
      .then((res: { items: Producto[]; total: number }) => {
        setRows(res.items);
        setTotal(res.total);
      })
      ...
      <div className="bg-[var(--page-bg)] rounded-xl px-[var(--content-x)] pb-[var(--content-b)] pt-3">
        ...
        <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button className="h-10 min-w-[140px] rounded-md border border-tg bg-tg-card px-3 text-left text-sm text-tg-muted">
              <span className="inline-flex items-center gap-2">
                <MaterialIcon name="expand_more" size={18} /> Activo
              </span>
            </button>
            <button className="h-10 min-w-[140px] rounded-md border border-tg bg-tg-card px-3 text-left text-sm text-tg-muted">
              <span className="inline-flex items-center gap-2">
                <MaterialIcon name="expand_more" size={18} /> Stock
              </span>
            </button>
                  ...
                  : rows.map((r) => (
                      <tr key={r.id} className="border-t border-tg hover:bg-black/5 dark:hover:bg-white/5">
                        ...
                        <td className="px-4 py-3">
                          {r.activo ? (
                            <span className="inline-flex items-center gap-1 rounded-full bg-green-600/15 px-2 py-0.5 text-xs font-medium text-green-700 dark:bg-green-400/15 dark:text-green-300">
                              <span className="h-1.5 w-1.5 rounded-full bg-green-600 dark:bg-green-300" />
                              Activo
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1 rounded-full bg-red-600/15 px-2 py-0.5 text-xs font-medium text-red-700 dark:bg-red-400/15 dark:text-red-300">
                        ...
                        <td className="px-4 py-3">
                          {format(new Date(r.fecha_creacion), "dd MMM yyyy", { locale: es })}
                        </td>
