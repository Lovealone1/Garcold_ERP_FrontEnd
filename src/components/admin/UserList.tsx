...
function roleOf(u: AdminUserOut): string {
    const hardcoded: Record<string, string> = {};
    return hardcoded[u.id] ?? "member";
}

function statusPill(confirmed_at?: string | null) {
    const ok = !!confirmed_at;
    const cls = ok
            ...
            <div className="mt-2 overflow-x-auto rounded-lg border border-tg">
                <table className="w-full border-collapse text-sm">
                    ...
                    <tbody className="bg-[var(--panel-bg)]">
                        {items.map((u) => {
                            ...
                            return (
                                <tr key={u.id} className="border-t border-[var(--panel-border)]">
                                    ...
                                    <td className="px-4 py-3 capitalize">{roleOf(u)}</td>
                                    <td className="px-4 py-3">{statusPill(u.confirmed_at)}</td>
                        ...
                        {!items.length && (
                            <tr>
                                <td colSpan={7} className="px-4 py-8 text-center text-tg-muted">
                                    Sin usuarios.
                                </td>
                            </tr>
                        )}
                        ...
                        <tr>
                            <td colSpan={7} className="px-4 py-3">
                                <div className="flex items-center gap-3 justify-end">
                                    ...
                                    <button className="px-2 py-1 text-sm rounded border border-tg" onClick={onPrev} disabled={page <= 1}>
                                        Prev
                                    </button>
                                    <button className="px-2 py-1 text-sm rounded border border-tg" onClick={onNext} disabled={!hasNext}>
                                        Next
                                    </button>
                                </div>
                            </td>
