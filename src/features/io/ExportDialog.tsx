...
type Props = {
    open: boolean;
    onClose: () => void;
    onDownload: (entity: Entity, fmt: Fmt, filename?: string) => Promise<boolean>;
    loading: boolean;
    error?: string | null;
    entities?: Entity[];
    fixedEntity?: Entity;
...
export default function ExportDialog({
    ...
    const [name, setName] = useState<string>(
        defaultName ?? String(initialEntity)
    );
    ...
    const handleDownload = () => {
        const filename = `${safeBaseName(name)}.${fmt}`;
        void onDownload(entity, fmt, filename);
    };
