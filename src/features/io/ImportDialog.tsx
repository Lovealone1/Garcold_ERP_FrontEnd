...
export default function ImportDialog({
    ...
    return (
            ...
            <div className="relative w-[560px] rounded-lg border border-tg bg-[var(--panel-bg)] shadow-xl">
                ...
                {loading && (
                    <div className="absolute left-0 top-0 h-[2px] w-full overflow-hidden" role="progressbar" aria-label="Cargando">
                        <div className="h-full w-1/3 bg-tg-primary animate-importbar" />
                    </div>
                )}
                ...
                <div className="px-4 py-4 space-y-3 text-sm">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <label className="mt-6 inline-flex items-center gap-2">
                            <input type="checkbox" checked={dryRun} onChange={() => setDryRun(v => !v)} disabled={loading} />
                    ...
                    {report ? (
                        <pre className="max-h-40 overflow-auto bg-black/10 p-2 rounded">{JSON.stringify(report, null, 2)}</pre>
                    ) : null}
                </div>
                ...
                <div className="px-4 py-3 border-t border-tg flex justify-end gap-2">
                    ...
                    <button
                        ...
                        className="h-9 rounded-md px-3 text-sm font-medium disabled:opacity-60
                       border border-tg-primary text-tg-primary bg-[var(--panel-bg)]
                       hover:bg-[color-mix(in_srgb,var(--tg-primary)_10%,transparent)]"
        ...
        .animate-importbar {
          animation: importbar 1.1s ease-in-out infinite;
        }
      `}</style>
        </div>
    );
