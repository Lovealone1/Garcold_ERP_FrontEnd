// src/features/productos/ProductImagesModal.tsx
"use client";
...
type Preview = { src: string; kind: "existing" | "file"; idx: number; id?: number };

const ACCEPT = "image/png,image/jpeg,image/webp";
const MAX_MB = 5;
const MAX_BYTES = MAX_MB * 1024 * 1024;

export default function ProductImagesModal({ open, onClose, productId, max = 3 }: Props) {
    ...
    const { deleteMedia: deleteMediaReq, loading: deletingLoading } = useDeleteMedia();
    ...
    const previews: Preview[] = useMemo(() => {
        const out: Preview[] = [];
        items.forEach((m, i) => out.push({ src: m.public_url, kind: "existing", idx: i, id: m.id }));
        files.forEach((f, i) => out.push({ src: URL.createObjectURL(f), kind: "file", idx: i }));
    ...
    function handlePickFiles(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        const valids = list.filter((f) => ACCEPT.includes(f.type) && f.size <= MAX_BYTES);
    ...
    async function handleSubmit() {
        ...
        setSubmitting(true);
        try {
            const added = await uploadProductFiles({ product_id: productId, files: filesToUpload });
    ...
    async function handleDelete(mediaId: number) {
        ...
        try {
            ...
            const d = e?.response?.data?.detail;
            alert(d ? (typeof d === "string" ? d : JSON.stringify(d)) : "Error al eliminar.");
        } finally {
            setDeletingId(null);
        }
    }
    ...
    function normalizeError(e: any): string | null {
        ...
        const msg =
            !d ? e?.message :
                typeof d === "string" ? d :
                    Array.isArray(d) ? d.map((x: any) => x?.msg ?? JSON.stringify(x)).join(" • ") :
                        typeof d === "object" ? (d.message ?? d.error ?? JSON.stringify(d)) :
                            String(d);
        ...
        >
                ...
                {fetching ? (
                    ...
                    <>
                        <Grid container spacing={2} sx={{ mb: 2 }}>
                            ...
                            {fetchError && (
                                <Grid size={{ xs: 12 }}>
                                    <Box sx={{ color: "error.main", fontSize: 13 }}>{fetchError}</Box>
                                </Grid>
                            )}
                        </Grid>
                        ...
                        <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
                            <Button
                                ...
                                sx={{
                                    ...
                                    "&:hover": { bgcolor: "color-mix(in srgb, var(--tg-card-bg) 85%, black 15%)" },
                                }}
                            >
                                Seleccionar archivos…
                                <input type="file" accept={ACCEPT} multiple hidden onChange={handlePickFiles} />
                            </Button>
                        ...
                        {error && (
                            <Box sx={{ color: "error.main", fontSize: 5, mt: 1 }}>{errText}</Box>
                        )}
                    </>
                )}
            </DialogContent>
            ...
            <DialogActions sx={{ px: 3, py: 2 }}>
                ...
                <Button
                    ...
                    startIcon={busy ? <CircularProgress size={16} sx={{ color: "var(--tg-primary-fg)" }} /> : undefined}
                >
