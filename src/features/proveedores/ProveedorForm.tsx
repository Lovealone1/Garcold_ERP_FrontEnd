...
export default function ProveedorForm(props: Props) {
  ...
  return mode === "page" ? (
    <Box
      sx={{ p: 3, borderRadius: 2, border: "1px solid var(--panel-border)", bgcolor: "var(--tg-card-bg)", color: "var(--tg-card-fg)" }}
    >
      ...
      <Stack direction="row" justifyContent="flex-end" gap={1.5} sx={{ pt: 2 }}>
        ...
        >
          {loading ? "Guardando…" : isCreate ? "Crear proveedor" : "Guardar cambios"}
        </Button>
      </Stack>
    </Box>
  ) : (
    <Dialog
      ...
      maxWidth="md"
      slotProps={{
        paper: { sx: { borderRadius: 2, border: "1px solid var(--panel-border)", bgcolor: "var(--tg-card-bg)", color: "var(--tg-card-fg)" } },
      }}
    >
      <DialogTitle sx={{ fontWeight: 600, pb: 1.5 }}>{isCreate ? "Nuevo Proveedor" : "Editar Proveedor"}</DialogTitle>
